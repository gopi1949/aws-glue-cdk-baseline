...
class PipelineStack(Stack):
    ...
    def __init__(self, scope: Construct, construct_id: str, config: Dict, **kwargs) -> None:
        ...
        # GitHub source (using a personal access token stored in Secrets Manager)
        source = CodePipelineSource.git_hub(
            "gopi1949/aws-glue-cdk-baseline",
            "main",
            authentication=SecretValue.secrets_manager("github-token")
        )

        # Custom CodeBuild project using external buildspec.yml
        ...
        dev_stage_instance.add_post(
            CodeBuildStep("IntegrationTest",
                input=source,
                commands=[
                    f"python $(pwd)/tests/integ/integ_test_glue_app_stack.py --account {str(config['devAccount']['awsAccountId'])} --region {config['devAccount']['awsRegion']} --stage-name DeployDev --sts-role-arn {dev_stage.iam_role_arn}"
                ],
                role_policy_statements=[
                    iam.PolicyStatement(
